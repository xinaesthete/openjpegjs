...
RUN apt-get update \
    ...
    #
    # Install C++ tools and TypeScript
...
# Install Node.js 22 and TypeScript globally
RUN su - $USERNAME -c "source ~/.nvm/nvm.sh && nvm install 22 && nvm use 22 && nvm alias default 22 && npm install -g typescript" \
    && ln -s /home/$USERNAME/.nvm/versions/node/v22.*/bin/tsc /usr/local/bin/tsc
